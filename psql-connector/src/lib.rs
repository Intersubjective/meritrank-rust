...
lazy_static! {
  ...
  static ref RECV_TIMEOUT_MSEC: u64 =
    var("MERITRANK_RECV_TIMEOUT_MSEC").ok().and_then(|s| s.parse::<u64>().ok()).unwrap_or(10000);
}
...
fn request_raw(
  ...
  client.send(Message::from(payload.as_slice())).map_err(|(_, err)| err)?;
  return Ok(client.recv()?);
}

fn request<T>(payload: Vec<u8>, timeout_msec: Option<u64>) -> Result<T, Box<dyn Error + 'static>>
where
...
mod tests {
  ...
  fn zerorec_graph_all() {
    ...
    let res =
      crate::mr_graph(Some("Uadeb43da4abb"), Some("U000000000000"), None, Some(false), None, None)
        .unwrap();
  ...
  fn service() {
    ...
    assert_eq!(ver.split(".").map(|x| x.parse::<i32>().unwrap()).count(), 3);
  }
  ...
  fn new_edges_fetch() {
    ...
    assert_eq!(crate::mr_fetch_new_edges(Some("U1"), Some("B")).unwrap().count(), 0);
