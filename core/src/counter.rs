...
impl Counter {
  ...
  pub fn increment_unique_counts<'a, I>(
    &mut self,
    items: I,
  ) where
    I: IntoIterator<Item = &'a NodeId>,
  {
    let unique_values: SetUsize =
      SetUsize::from_iter(items.into_iter().copied());
    self.increment_counts(unique_values);
  }

  pub fn decrement_unique_counts<'a, I>(
    &mut self,
    items: I,
  ) where
    I: IntoIterator<Item = &'a NodeId>,
