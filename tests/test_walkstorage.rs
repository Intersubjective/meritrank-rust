...
mod tests {
    ...
    fn test_walk_storage_add_walk() {
        ...
        let expected_walks_str = format!("WalkStorage {{ walks: {:?} }}", indexmap! {
...
// }
    ...
    fn test_walk_storage_get_walks_starting_from_node() {
        ...
        let walk1 = RandomWalk::from_nodes(vec![
            NodeId::from(1),
            NodeId::from(2),
            NodeId::from(3),
        ]);
        let walk2 = RandomWalk::from_nodes(vec![
            NodeId::from(1),
            NodeId::from(4),
            NodeId::from(5),
        ]);
    ...
    fn test_walk_storage_drop_walks_from_node() {
        ...
        let walk3 = RandomWalk::from_nodes(vec![
            NodeId::from(2),
            NodeId::from(3),
            NodeId::from(4),
        ]);
        ...
        assert_eq!(walk_storage.get_walks().get(&NodeId::from(1)).map(|m| format!("{:?}", m)), None.map(|()| "".to_string()));
    ...
    fn test_walk_storage_get_walks_through_node() {
        ...
        let walks_filter_len = walk_storage.get_walks_through_node(NodeId::from(1), |pos_walk| pos_walk.get_walk().len() > 2);
        ...
        let walks_filter_node = walk_storage.get_walks_through_node(NodeId::from(2), |pos_walk| {
            pos_walk.get_current_node() == NodeId::from(2) && pos_walk.get_walk().contains(&NodeId::from(2))
        });
        ...
        let walks_no_match = walk_storage.get_walks_through_node(NodeId::from(3), |pos_walk| pos_walk.get_current_node() == NodeId::from(5));
    ...
    fn test_walk_storage_decide_skip_invalidation() {
        ...
        let (may_skip, new_pos) = storage.decide_skip_invalidation(&walk, 2, edge, 0.0, Some(&mut rng));
        assert!(may_skip);
        assert_eq!(new_pos, 2);

        // Test skipping invalidation with step recalculation probability
        let storage = WalkStorage::new();
        let (may_skip, new_pos) = storage.decide_skip_invalidation(&walk, 1, edge, step_recalc_probability, Some(&mut rng));
        assert!(may_skip);
        assert_eq!(new_pos, 1);

        // Test invalidation without skipping
        let storage = WalkStorage::new();
        let (may_skip, new_pos) = storage.decide_skip_invalidation(&walk, 0, edge, step_recalc_probability, Some(&mut rng));
        assert!(!may_skip);
        assert_eq!(new_pos, 1);
    }
    ...
    fn test_walk_storage_decide_skip_invalidation_on_edge_addition() {
        ...
        // Test invalidation without skipping
        let (may_skip, new_pos) = storage.decide_skip_invalidation_on_edge_addition(&walk, 0, edge, step_recalc_probability, Some(&mut rng));
        assert!(!may_skip);
        assert_eq!(new_pos, 2);

        // Test invalidation with skipping
        let (may_skip, new_pos) = storage.decide_skip_invalidation_on_edge_addition(&walk, 2, edge, step_recalc_probability, Some(&mut rng));
        assert!(may_skip);
        assert_eq!(new_pos, 2);

        // Test invalidation with step recalculation
        let (may_skip, new_pos) = storage.decide_skip_invalidation_on_edge_addition(&walk, 1, edge, step_recalc_probability, Some(&mut rng));
