...
impl VSIDSManager {
  ...
  /// Обновить вес ребра с учётом bump (экспоненты)
  pub fn update_weight(
    ...
    // Нормализация, если превышен порог
  ...
  /// Найти максимальный вес для всех рёбер из одного узла
  fn max_source_weight(
    &self,
    ctx: &str,
    src: &str,
  ) -> Option<f64> {
  ...
  /// Нормализовать веса всех рёбер из одного узла
  fn normalize(
    &mut self,
    ctx: &str,
    src: &str,
...
mod tests {
  ...
  fn test_normalization() {
    ...
    // assert!((weight_c / weight_b - 1.0 / 1e16).abs() < f64::EPSILON);
  }
