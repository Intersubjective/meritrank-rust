...
impl VSIDSManager {
    ...
    pub fn get_weight(&self, ctx: &str, src: &str, dst: &str) -> Option<f64> {
        self.weights.get(&(ctx.to_string(), src.to_string(), dst.to_string())).copied()
    }
    ...
    fn normalize(&mut self, ctx: &str, src: &str) {
        ...
        if let Some(&max_weight) = self.max_indices.get(&source_key) {
            if max_weight > 0.0 {
                let normalized: Vec<(Edge, f64)> = self.weights
                    .iter()
    ...
    fn cleanup_small_edges(&mut self, ctx: &str, src: &str, max_weight: f64) {
        let threshold = max_weight * self.deletion_ratio;

        let keys_to_remove: Vec<Edge> = self.weights
            .iter()
            .filter(|((c, s, _), &weight)| c == ctx && s == src && weight <= threshold)
...
mod tests {
    ...
    fn test_normalization() {
        ...
        assert!(mgr.get_weight("test", "A", "B").unwrap() <= mgr.max_threshold,
