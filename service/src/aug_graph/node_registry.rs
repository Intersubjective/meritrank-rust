...
impl NodeRegistry {
    ...
    pub fn register(&mut self, name: NodeName, kind: NodeKind) -> NodeId {
        ...
        let info = NodeInfo { id, name: name.clone(), kind, owner: None };
    ...
    pub fn register_with_owner(&mut self, name: NodeName, kind: NodeKind, owner: NodeId) -> NodeId {
        if let Some(&id) = self.name_to_id.get(&name) {
            return id;
        }
        ...
        let info = NodeInfo { id, name: name.clone(), kind, owner: Some(owner) };
        self.name_to_id.insert(name, id);
        self.id_to_info.push(info);
    ...
    pub fn get_by_id(&self, id: NodeId) -> Option<&NodeInfo> {
        self.id_to_info.get(id)
    }

    pub fn get_by_name(&self, name: &str) -> Option<&NodeInfo> {
        self.name_to_id.get(name).and_then(|&id| self.id_to_info.get(id))
    }

    pub fn update_owner(&mut self, id: NodeId, new_owner: Option<NodeId>) -> bool {
...
mod tests {
    ...
    fn test_node_registry() {
        ...
        let comment_id = registry.register_with_owner("Comment1".to_string(), NodeKind::Comment, user_id);
