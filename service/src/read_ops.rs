...
impl AugMultiGraph {
  ...
  fn handle_poll_variant_case(
    ...
    // Get all poll variants for this poll
    let poll_variants = subgraph
      .poll_store
      .polls
      .get(&focus_id)
      .cloned()
    ...
    // Get poll votes if they exist
    let poll_votes = subgraph
      .poll_store
      .votes
      .get(&focus_id)
      .cloned()
    ...
    let mut results: Vec<_> = poll_variants
      .into_iter()
      .map(|option_id| {
        let pers_poll_variant_votes_share = personalized_poll_result
          ...
          .cloned()
          .unwrap_or(0.0);
        let global_poll_variant_votes_share =
          global_poll_results.get(&option_id).cloned().unwrap_or(0.0);
