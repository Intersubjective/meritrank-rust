...
fn read_csv_from_tar_gz(tar_gz_path: &str, target_file: &str) -> Vec<Edge> {
...
fn put_testing_edges(graph: &mut AugMultiGraph, file_name: &str) {
    for edge in read_csv_from_tar_gz("src/edges.tar.gz", file_name) {
        graph.write_put_edge(&edge.context, &edge.src, &edge.dst, edge.weight, edge.magnitude);
    }
}
