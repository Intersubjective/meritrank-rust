...
fn perform_command(data: &Data, command: Command) -> Result<Vec<u8>, ()> {
...
fn put_for_write(data: &Data, command: Command) {
...
fn decode_and_handle_request(data: &Data, request: &[u8]) -> Result<Vec<u8>, ()> {
...
fn worker_callback(data: &Data, aio: Aio, ctx: &Context, res: AioResult) {
