...
use std::{collections::HashMap, env::var, string::ToString, sync::atomic::Ordering};
...
use meritrank_core::{constants::EPSILON, Graph, MeritRank, MeritRankError, NodeId};
use petgraph::{
  graph::{DiGraph, NodeIndex},
  visit::EdgeRef,
};
...
pub fn bloom_filter_bits(size: usize, num_hashes: usize, id: usize) -> Vec<u64> {
...
pub fn bloom_filter_add(mask: &mut [u64], bits: &[u64]) {
...
pub fn bloom_filter_contains(mask: &[u64], bits: &[u64]) -> bool {
  if mask.len() != bits.len() {
...
impl AugMultiGraph {
  pub fn cache_score_add(&mut self, context: &str, ego: NodeId, dst: NodeId, score: Weight) {
    ...
    for (i, x) in self.cached_scores.iter().enumerate().rev() {
      if x.ego == ego && x.dst == dst && x.context == context {
        ...
        self.cached_scores.push(CachedScore { context: context.to_string(), ego, dst, score });
        return;
      }
    }
  ...
  pub fn cache_score_get(&mut self, context: &str, ego: NodeId, dst: NodeId) -> Option<Weight> {
  ...
  pub fn cache_walk_add(&mut self, context: &str, ego: NodeId) {
    ...
    for (i, x) in self.cached_walks.iter().enumerate().rev() {
      if x.ego == ego && x.context == context {
        ...
        self.cached_walks.push(CachedWalk { context: context.to_string(), ego });
        return;
      }
    }
  ...
  pub fn cache_walk_get(&mut self, context: &str, ego: NodeId) -> bool {
...
impl AugMultiGraph {
  ...
  pub fn is_user_edge(&mut self, src: NodeId, dst: NodeId) -> bool {
  ...
  fn fetch_all_scores(&mut self, context: &str, ego_id: NodeId) -> Vec<(NodeId, Weight)> {
  ...
  fn fetch_score(&mut self, context: &str, ego_id: NodeId, dst_id: NodeId) -> Weight {
  ...
  fn fetch_score_reversed(&mut self, context: &str, dst_id: NodeId, ego_id: NodeId) -> Weight {
  ...
  fn fetch_user_score_reversed(&mut self, context: &str, dst_id: NodeId, ego_id: NodeId) -> Weight {
    ...
    match self.graph_from(context).graph.get_node_data(ego_id) {
      Some(x) => {
        if x.pos_edges.len() + x.neg_edges.len() == 0 {
          ...
          let parent_id =
            if x.pos_edges.len() > 0 { x.pos_edges.keys()[0] } else { x.neg_edges.keys()[0] };
  ...
  pub fn find_or_add_node_by_name(&mut self, node_name: &str) -> NodeId {
  ...
  pub fn set_edge(&mut self, context: &str, src: NodeId, dst: NodeId, amount: f64) {
...
impl AugMultiGraph {
  pub fn read_node_score(
    ...
    [(ego.to_string(), target.to_string(), score_of_target_from_ego, score_of_ego_from_target)]
      .to_vec()
  }
  ...
  pub fn write_put_edge(&mut self, context: &str, src: &str, dst: &str, amount: f64) {
  ...
  pub fn write_delete_edge(&mut self, context: &str, src: &str, dst: &str) {
  ...
  pub fn write_delete_node(&mut self, context: &str, node: &str) {
  ...
  pub fn read_graph(
      ...
      if dst_kind == NodeKind::User {
        ...
        for (ngh_id, dst_ngh_weight) in dst_neighbors {
          ...
          let focus_ngh_weight = focus_dst_weight
            * dst_ngh_weight
            * if focus_dst_weight < 0.0 && dst_ngh_weight < 0.0 { -1.0 } else { 1.0 };
    ...
    if ego_id == focus_id {
      ...
      for _ in 0..10000 {
        ...
        match status.clone() {
          Status::NEIGHBOR(request) => match graph_cloned.get_node_data(request.node) {
            None => neighbor = None,
            Some(data) => {
      ...
      for k in 0..ego_to_focus.len() - 1 {
        ...
        if k + 2 == ego_to_focus.len() {
          ...
          let a_c_weight =
            a_b_weight * b_c_weight * if a_b_weight < 0.0 && b_c_weight < 0.0 { -1.0 } else { 1.0 };
    ...
    for (_, src_index) in indices.iter() {
      let neighbors: Vec<_> =
        im_graph.edges(*src_index).map(|edge| (edge.target(), edge.id())).collect();
  ...
  pub fn read_connected(&mut self, context: &str, ego: &str) -> Vec<(String, String)> {
  ...
  pub fn read_node_list(&self) -> Vec<(String,)> {
    ...
    self.node_infos.iter().map(|info| (info.name.clone(),)).collect()
  }
  ...
  fn kmeans_plus_plus_initialization(
    ...
    for _ in 1..k {
      // Update the minimum distance for each point to the closest centroid
      for (i, &point) in data.iter().enumerate() {
        let dist = Self::distance(
          &(point.0, point.1),
          &(centroids.last().unwrap().0 as usize, centroids.last().unwrap().1 as usize),
        );
  ...
  pub fn kmeans(
    ...
    if data.is_empty() {
      log::error!("Data points are empty");
      return vec![];
    }
    ...
    for _ in 0..max_iterations {
      ...
      for (i, &point) in data.iter().enumerate() {
        let (closest, _) = centroids
          ...
          .enumerate()
          .map(|(j, &centroid)| {
            (j, Self::distance(&(point.0, point.1), &(centroid.0 as usize, centroid.1 as usize)))
          })
      ...
      // Early stopping based on centroid movement
  ...
  pub fn read_mutual_scores(
    ...
    for (node, score) in ranks {
      ...
      if score > 0.0 {
        ...
        points.push((score as usize, reversed_score as usize));
      }
    }

    // Apply k-means clustering to the points (score, reversed_score)
    ...
    // println!("Clusters: {:#?}", clusters.clone());
    println!(
      "Your scores are: {:#?}",
      v.clone()
        .into_iter()
  ...
  pub fn read_new_edges_filter(&mut self, src: &str) -> Vec<u8> {
  ...
  pub fn write_new_edges_filter(&mut self, src: &str, filter_bytes: &[u8]) {
  ...
  pub fn write_fetch_new_edges(&mut self, src: &str, prefix: &str) -> Vec<(String, Weight)> {
