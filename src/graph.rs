...
#[allow(dead_code)]
impl<NodeData : Copy + Default> Graph<NodeData> {
  ...
  pub fn add_node(
    ...
    self.nodes.insert(node, (index, data));
    index
  }
  ...
  pub fn update_index(&mut self) {
    //  FIXME
    //  Skip when NodeData is empty type.
    let all_data : HashMap<NodeIndex, NodeData> = self.nodes
      .iter()
    ...
    self.nodes = self.graph
      .node_indices()
      .map(|index| (self.graph[index], (index, *all_data.get(&index).unwrap_or(&NodeData::default()))))
      .collect();
  }
  ...
  // Experimental
  pub fn add_node_by_id(&mut self, node_id : NodeId, data : NodeData) -> NodeIndex {
    // Add a node to the graph and store its NodeIndex in the nodes mapping
    let index = self.graph.add_node(node_id);
    self.nodes.insert(node_id, (index, data));
    index
  }
  ...
  pub fn upsert_edge_with_nodes(
    ...
    let _ = self.add_node_by_id_if_not_exists(source, src_data);
    let _ = self.add_node_by_id_if_not_exists(target, dst_data);
...
impl<NodeData : Copy + Default> PartialEq for Graph<NodeData> {
